...
- **`backfill.py`**: `BackfillScanner` walks the gateway's contiguous data filesystem, content-sniffs for HTML, scans through the rule engine + ML, caches verdicts, and blocks malicious content in enforce mode. Uses `GatewayDBReader` for read-only hash→TX ID lookups via the gateway's `data.db`. A malicious/suspicious hit whose hash has no TX-ID mapping can't be blocked (nothing to send to `block-data`); rather than leave a bare `tx_id='backfill'` sentinel that reads like a failed block, it's persisted with `source='backfill_unresolved'`, counted (`scanner_backfill_unresolved` / `backfill_unresolved_found` state), and logged (`backfill_detection_unresolvable`) — deduped by `has_verdict()`, and excluded from the unblocked-malicious audit by the existing `tx_id='backfill'` filter. **IPFS cached content is not covered by backfill** — it lives in a separate `data/ipfs-cache/` directory with a different layout (hash-based subdirs plus `.meta` companion files). IPFS content is still scanned via the on-access (`data-cached`) webhook path; a dedicated IPFS sweep is future work.
